...
export default class GameDeploymentRepository {
  ...
  public async getDeployment(): Promise<GameDeployment | null> {
    ...
    const rows = await this.dirtyMysqlAdapter.query(
      `
      SELECT gd.id as gd_id, gd.status as gd_status, gd.consumer_uuid as gd_consumer_uuid, gv.docker_image as gv_docker_image, ci.provider as ci_provider, ci.api_name as ci_api_name, ci.cost_per_hour as ci_cost_per_hour, ci.region as ci_region, gi.id as gi_id, gi.name as gi_name, gv.docker_image as gv_docker_image
